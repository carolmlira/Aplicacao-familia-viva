...
export class FirebaseController {
  ...
  async uploadMultipleFiles(
    ...
    if (category === 'pages' && !pageId) {
      throw new BadRequestException(
        'pageId é obrigatório para uploads na categoria "pages".',
      );
    }
  ...
  async updateFile(
    ...
    if (!category || !filename || !newName) {
      throw new BadRequestException(
        'Categoria, filename e newName são obrigatórios.',
      );
    }
    ...
    if (category === 'pages') {
      if (!pageId) {
        throw new BadRequestException(
          'pageId é obrigatório para categoria "pages".',
        );
      }
  ...
  async getPagesByCategory(@Query('category') category: string) {
    ...
    const snapshot = await this.firebaseService.getCollectionByPath(
  ...
  async deleteFileGeneric(
    ...
    if (!category || !filename) {
      throw new BadRequestException(
        'Parâmetros obrigatórios: category e filename',
      );
    }
    let fullPath: string;
  ...
  async deleteFolder(
    ...
    if (!category || !subgrup || !pageId) {
      throw new BadRequestException(
        'category, subgrup e pageId são obrigatórios.',
      );
    }
