...
export default function Home() {
  ...
  const [showModalProgramacaoEdit, setShowModalProgramacaoEdit] =
    useState(false);
  ...
  const settings = {
    ...
    infinite: true,
    speed: 500,
    slidesToShow: 4, // Ajuste conforme o design
    slidesToScroll: 1,
    responsive: [
      {
        breakpoint: 1024,
  ...
  useEffect(() => {
    const baseUrl = `${process.env.NEXT_PUBLIC_STORAGE_BASE}`;
    ...
    async function fetchProjects() {
      try {
        ...
        const data = await res.json();
        const rawProjects: PagesProject[] = Array.isArray(data.pages)
          ? data.pages
          : [];
    ...
    async function fetchFiles(category: string) {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/firebase/list?category=${category}`
        );
        const data = await res.json();
  ...
  async function handleSaveBanner() {
    try {
      const formData = new FormData();
      if (banner?.imagemLogoFile)
        formData.append("imagemLogo", banner.imagemLogoFile);
      if (banner?.imagemBannerFile)
        formData.append("imagemBanner", banner.imagemBannerFile);
      if (banner?.frase) formData.append("frase", banner.frase);
      if (!banner) return;

      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/banner/${banner.id}`,
        {
          method: "PATCH",
          body: formData,
        }
          ...
          <div className={styles.modalContentBanner}>
            ...
            <div className={styles.modalActionsBanner}>
              ...
              <button
                ...
                onClick={() => setShowModalBanner(false)}
              >
                Cancelar
              </button>
            </div>
        ...
        <div className={styles.modalOverlaySobre}>
          <div className={styles.modalContentSobre}>
            ...
            <input
              style={{ boxShadow: "0 4px 4px -2px rgba(0, 0, 0, 0.3)" }}
              type="file"
              onChange={handleSobreImageChange}
            />
            {editSobre.imagem && (
              <Image
                src={
            ...
            <div className={styles.modalActionsBanner}>
              <button
                ...
                onClick={() => handleSaveSobre(editSobre.id)}
              >
                Salvar
              </button>
              <button
                ...
                onClick={() => setShowModalEditSobre(false)}
              >
                Cancelar
              </button>
            </div>
      ...
      {isAdmin && (
        <div className="container-programacao-add">
          <button
            ...
            onClick={() => setShowModalProgramacaoAdd(true)}
          >
            <Image
          ...
          <div className={styles.modalContentEvent}>
            ...
            <div className={styles.modalSectionEvent}>
              ...
              <input
                ...
                value={newEvent.days}
                onChange={(e) =>
                  setNewEvent({ ...newEvent, days: e.target.value })
                }
            ...
            <div className={styles.modalActionsBanner}>
              ...
              <button
                ...
                onClick={() => setShowModalProgramacaoAdd(false)}
              >
                Cancelar
              </button>
            </div>
        ...
        <div className={styles.modalOverlaySobre}>
          <div className={styles.modalContentSobre}>
            ...
            <input
              ...
              style={{ boxShadow: "0 4px 4px -2px rgba(0, 0, 0, 0.3)" }}
              onChange={(e) =>
                setFormEdit({ ...formEdit, time: e.target.value })
              }
            />
            ...
            <div className={styles.modalActionsBanner}>
              ...
              <button
                ...
                onClick={() => setShowModalProgramacaoEdit(false)}
              >
                Cancelar
              </button>
            </div>
