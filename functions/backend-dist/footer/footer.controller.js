"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FooterController = void 0;
const common_1 = require("@nestjs/common");
const footer_service_1 = require("./footer.service");
const update_footer_1 = require("./dto/update-footer/update-footer");
const create_footer_1 = require("./dto/create-footer/create-footer");
let FooterController = class FooterController {
    footerService;
    constructor(footerService) {
        this.footerService = footerService;
    }
    async getFooter() {
        return await this.footerService.getFooter();
    }
    async updateFooter(updateDto) {
        return await this.footerService.updateFooter(updateDto);
    }
    async createFooter(createDto) {
        return await this.footerService.createFooter(createDto);
    }
};
exports.FooterController = FooterController;
__decorate([
    (0, common_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], FooterController.prototype, "getFooter", null);
__decorate([
    (0, common_1.Patch)(),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [update_footer_1.UpdateFooter]),
    __metadata("design:returntype", Promise)
], FooterController.prototype, "updateFooter", null);
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_footer_1.CreateFooter]),
    __metadata("design:returntype", Promise)
], FooterController.prototype, "createFooter", null);
exports.FooterController = FooterController = __decorate([
    (0, common_1.Controller)('footer'),
    __metadata("design:paramtypes", [footer_service_1.FooterService])
], FooterController);
//# sourceMappingURL=footer.controller.js.map